...
describe('LendingApiClient', () => {
  ...
  test('refuses raw Avalanche account submission before wallet broadcast', async () => {
    const sendTransactionMock = vi.fn().mockResolvedValue({
      transactionHash:
        '0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    });

    safeExecuteTransactionV2Mock.mockImplementation(
  ...
  test('refuses raw Avalanche provider submission before eth_sendTransaction', async () => {
  ...
  test('does not run legacy Avalanche hash recovery after policy refusal', async () => {
    const sendTransactionMock = vi.fn();
    safeExecuteTransactionV2Mock.mockImplementation(
      async (fn: () => Promise<{ transactionHash: string }>) => {
        try {
