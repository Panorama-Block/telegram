...
import { TelegramAuthHandler } from '../../../utils/telegramAuthHandler';
import { setupTelegramAuthEnvironment } from '../../../config/telegramAuth';
...
export function TelegramCompatibleWallet() {
  ...
  useEffect(() => {
    // Configurar ambiente de autenticação para miniapps
    setupTelegramAuthEnvironment();

    const handleAuthCallback = () => {
      // Verificar se estamos no callback de autenticação
      if (typeof window !== 'undefined' && window.location.pathname === '/auth/callback') {
        // Aguardar o Thirdweb processar o callback
        setTimeout(() => {
          const authToken = localStorage.getItem('authToken');
          if (authToken) {
            setIsAuthenticated(true);
            setAuthMessage('Autenticação concluída!');
    ...
    // Configurar handler para interceptar Google OAuth
    const authHandler = TelegramAuthHandler.getInstance();
    authHandler.onAuthResult((success, data) => {
      if (success) {
        setIsAuthenticated(true);
        setAuthMessage('Autenticação concluída!');
        if (data?.token) {
...
export default TelegramCompatibleWallet;
