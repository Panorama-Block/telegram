...
import { AVAX_CHAIN_ID, AVAX_LP_ENDPOINTS } from './config';
import type {
  AvaxLpPool,
  AvaxLpUserPosition,
...
const AvaxTokenInfoSchema = z.object({
  symbol: z.string(),
  address: z.string(),
  decimals: z.number(),
...
const AvaxLpPoolSchema = z.object({
  poolId: z.number(),
  name: z.string(),
  tokenA: AvaxTokenInfoSchema,
  tokenB: AvaxTokenInfoSchema,
  lpTokenAddress: z.string(),
  farmAddress: z.string().nullable(),
  estimatedAPR: z.string().nullable().default(null),
  totalLiquidityUsd: z.string().nullable().default(null),
  totalStaked: z.string(),
  rewardToken: AvaxTokenInfoSchema.nullable().default(null),
});

const AvaxLpUserPositionSchema = z.object({
  poolId: z.number(),
  poolName: z.string(),
  lpTokenAddress: z.string(),
  tokenA: AvaxTokenInfoSchema,
...
const AvaxLpPrepareResponseSchema = z.object({
  bundle: z.object({
...
function validateResponse<T>(schema: z.ZodType<T>, data: unknown, context: string): T {
  ...
  if (!result.success) {
    console.warn(`[AvaxLp] Invalid response shape for ${context}:`, result.error.flatten());
    return data as T;
  }
  return result.data;
}

class AvaxLpApiClient {
  ...
  constructor(account: any, switchChain?: SwitchChainFn, activeWallet?: any) {
    ...
    this.baseUrl = direct && direct.length > 0 ? direct.replace(/\/+$/, '') : '/api/yield';
  }
  ...
  private async recoverRecentTxHashByPayload(params: {
    ...
    const { provider, expectedChainId, from, to, data, timeoutMs = 25_000, lookbackBlocks = 12 } = params;
    ...
    while (Date.now() - startedAt < timeoutMs) {
      try {
        ...
        if (chainId != null && chainId !== expectedChainId) return null;
  ...
  private async fetchJson<T>(path: string, options?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`AvaxLp API error ${res.status}: ${details || res.statusText || 'Request failed'}`);
    }
    return res.json();
  }

  async getPools(): Promise<AvaxLpPool[]> {
    const data = await this.fetchJson<{ pools: AvaxLpPool[] }>(AVAX_LP_ENDPOINTS.POOLS);
    const validated = validateResponse(
      z.object({ pools: z.array(AvaxLpPoolSchema) }),
      data,
      'AvaxLpPools',
    );
    return validated.pools as AvaxLpPool[];
  }

  async getPosition(userAddress: string): Promise<AvaxLpUserPosition[]> {
    const data = await this.fetchJson<{ positions: AvaxLpUserPosition[] }>(
      `${AVAX_LP_ENDPOINTS.POSITION}/${userAddress}`,
    );
    const validated = validateResponse(
      z.object({ positions: z.array(AvaxLpUserPositionSchema) }),
      data,
      'AvaxLpPositions',
    );
  ...
  async prepareAddLiquidity(params: {
    ...
    const raw = await this.fetchJson<AvaxLpPrepareResponse>(AVAX_LP_ENDPOINTS.PREPARE_ADD, {
      method: 'POST',
      body: JSON.stringify(params),
    });
    return validateResponse(AvaxLpPrepareResponseSchema, raw, 'AvaxLpPrepareAdd');
  }

  async prepareRemoveLiquidity(params: {
    userAddress: string;
    tokenA: string;
    tokenB: string;
    lpAmount: string;
  }): Promise<AvaxLpPrepareResponse> {
    const raw = await this.fetchJson<AvaxLpPrepareResponse>(AVAX_LP_ENDPOINTS.PREPARE_REMOVE, {
      method: 'POST',
      body: JSON.stringify(params),
    });
    return validateResponse(AvaxLpPrepareResponseSchema, raw, 'AvaxLpPrepareRemove');
  }

  async prepareStake(params: {
    userAddress: string;
    poolId: number;
    lpAmount: string;
  }): Promise<AvaxLpPrepareResponse> {
    const raw = await this.fetchJson<AvaxLpPrepareResponse>(AVAX_LP_ENDPOINTS.PREPARE_STAKE, {
      method: 'POST',
      body: JSON.stringify(params),
    });
    return validateResponse(AvaxLpPrepareResponseSchema, raw, 'AvaxLpPrepareStake');
  }

  async prepareUnstake(params: {
    userAddress: string;
    poolId: number;
    lpAmount: string;
    proxyAddress?: string;
  }): Promise<AvaxLpPrepareResponse> {
    const raw = await this.fetchJson<AvaxLpPrepareResponse>(AVAX_LP_ENDPOINTS.PREPARE_UNSTAKE, {
      method: 'POST',
      body: JSON.stringify(params),
    });
    return validateResponse(AvaxLpPrepareResponseSchema, raw, 'AvaxLpPrepareUnstake');
  }

  async prepareClaimRewards(params: {
    userAddress: string;
    poolId: number;
    proxyAddress?: string;
  }): Promise<AvaxLpPrepareResponse> {
    const raw = await this.fetchJson<AvaxLpPrepareResponse>(AVAX_LP_ENDPOINTS.PREPARE_CLAIM, {
      method: 'POST',
      body: JSON.stringify(params),
    });
  ...
  private async _doExecute(tx: PreparedTransaction): Promise<TransactionExecutionStatus> {
    if (!this.account?.address) throw new Error('Wallet not connected');

    if (this.switchChain) {
      try {
        await this.switchChain(defineChain(tx.chainId || AVAX_CHAIN_ID));
      } catch { /* ignore if already on chain */ }
    }

    if (!this.account.sendTransaction) throw new Error('Wallet does not support sendTransaction');

    const targetChainId = Number(tx.chainId || AVAX_CHAIN_ID);
    ...
    const executionAttempt = safeExecuteTransactionV2(async () => {
      ...
      const recoverHashFromWallet = async (options?: { timeoutMs?: number; lookbackBlocks?: number }) =>
        this.recoverRecentTxHashByPayload({
      ...
      const raceBroadcastWithRecovery = async (
        ...
        let settled = false;
        const sendPromise = requestWithTimeout(Promise.resolve(send()), 45_000, 'Wallet transaction broadcast')
        ...
        const recoveryPromise = (async () => {
          ...
          while (!settled && Date.now() - startedAt < 55_000) {
            const recoveredHash = await recoverHashFromWallet({ timeoutMs: 2_500, lookbackBlocks: 24 });
        ...
        const raceResult = await Promise.race([
          ...
          (async () => {
            ...
            if (!recoveredHash) return await new Promise<never>(() => {});
            settled = true;
        ...
        if (directHash) return { transactionHash: directHash, source: 'wallet' };

        const recoveredAfterSend = await recoverHashFromWallet({ timeoutMs: 8_000, lookbackBlocks: 24 });
        if (recoveredAfterSend) return { transactionHash: recoveredAfterSend, source: 'recovered' };
      ...
      try {
        ...
        if (recoveredAccountHash) return { transactionHash: recoveredAccountHash, source: 'recovered' };
    ...
    try {
      result = await requestWithTimeout(executionAttempt, 70_000, 'Wallet confirmation');
      ...
      if (!isTimeout) throw executionError;
      ...
      if (recoveredHash) {
        console.warn('[AvaxLp] Wallet confirmation timed out, but tx hash was recovered:', recoveredHash);
        return { transactionHash: recoveredHash, confirmed: false, source: 'recovered' };
      }

      throw new Error(`${timeoutMessage}. If you already approved in wallet, check explorer and then press "Try again".`);
    } finally {
    ...
    return { transactionHash: result.transactionHash, confirmed: false, source: result.source };
  }
}

export function useAvaxLpApi() {
  const account = useActiveAccount();
  const switchChain = useSwitchActiveWalletChain();
  const activeWallet = useActiveWallet();

  return useMemo(
    () => new AvaxLpApiClient(account, switchChain, activeWallet),
    [account, switchChain, activeWallet],
  );
}

export { AvaxLpApiClient };
