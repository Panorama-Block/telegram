...
interface LiquiditySuccessCardProps {
...
export function LiquiditySuccessCard({
  txHashes,
  positionId,
  token0Symbol,
  token1Symbol,
  ...
  return (
    ...
    >
      ...
      <div className={cn('space-y-3', isCompact && 'space-y-2')}>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            >
              Your {token0Symbol}/{token1Symbol} position is now active. Fees will accrue automatically.
            </p>
          </div>
          {onClose && (
            <button
      ...
      {/* Position ID */}
      {positionId && !isCompact && (
        <div className="mt-6 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-pano-text-muted">
            Position ID
          </p>
          <div className="rounded-xl border border-pano-primary/20 bg-pano-primary/5 p-4">
            <code className="block text-sm font-mono font-semibold text-pano-primary break-all">
              {positionId}
            </code>
          </div>
        </div>
      ...
      {!isCompact && (
        <div className="mt-6 rounded-xl border border-pano-border-subtle bg-pano-bg-tertiary/80 p-4">
          ...
          <ul className="mt-2 space-y-1 text-xs text-pano-text-secondary">
            <li className="flex items-start gap-2">
              <span className="text-pano-primary">•</span>
              <span>Your liquidity is now earning fees from trades in this pool</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-pano-primary">•</span>
              <span>Monitor your position&apos;s performance in the Portfolio section</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-pano-primary">•</span>
              <span>You can remove liquidity at any time by closing the position</span>
            </li>
          </ul>
        </div>
      )}
...
export default LiquiditySuccessCard;
