...
import type { AvaxLpAction, AvaxLpPool, AvaxLpPrepareResponse } from '@/features/avax-lp/types';
...
export function AvaxLpReviewView({
  action,
  pool,
  amountA,
  amountB,
    ...
    return (
      ...
      >
        <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 py-12">
          ...
          <button onClick={onBack} className="text-sm text-zinc-400 hover:text-white transition-colors mt-2">
            Go Back
          </button>
        </div>
      </motion.div>
  ...
  if (!pool) {
    return (
      ...
      >
        <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 py-12">
          <AlertCircle className="w-10 h-10 text-red-400" />
          <p className="text-sm text-red-400 text-center">Pool context unavailable. Please go back and select the pool again.</p>
          <button onClick={onBack} className="text-sm text-zinc-400 hover:text-white transition-colors mt-2">
            Go Back
          </button>
        </div>
      ...
      <div className="px-4 sm:px-6 pb-6 space-y-3 relative z-10 flex-1 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-3 space-y-2">
          ...
          <div className="flex justify-between text-xs">
            <span className="text-zinc-500">LP token</span>
            <span className="text-zinc-300 font-mono">{truncateAddress(lpTokenAddress ?? pool.lpTokenAddress)}</span>
          </div>
          {farmAddress && (
            <div className="flex justify-between text-xs">
              <span className="text-zinc-500">Farm</span>
              <span className="text-zinc-300 font-mono">{truncateAddress(farmAddress)}</span>
            </div>
          )}
          <div className="flex justify-between text-xs">
          ...
          <div className="flex justify-between text-xs">
            ...
            <span className="text-zinc-300">Avalanche C-Chain</span>
          </div>
        </div>
