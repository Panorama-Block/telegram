// Temporarily commented out due to thirdweb locale import issues
 ...
 *
 * NOTE: Temporarily returning a default locale object until thirdweb locale imports are fixed
 */
export async function getInAppWalletLocale(localeId: string) {
  // Return a basic default locale structure
  return {
    connectWallet: {
      defaultButtonTitle: "Connect Wallet",
      // Add more default strings as needed
    }
  };

  // Original switch commented out
  // switch (localeId) {
  //   case 'es_ES':
  //     return es;
  //   case 'ja_JP':
